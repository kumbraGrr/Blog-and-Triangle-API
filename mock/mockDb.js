...
var mockDb = mysql.createConnection(dbinfo);


mockDb.connect(function(err) {
    if (err) {
...
mockDb.on('connection', function (connection) {
...
/* var dbCreation = () => {
    dbName = "mock";
     ...
     //Please insert desired name here
    if (err){
        throw err;  
    } 
    else {
    ...
    dbAssignment.query(`DROP DATABASE IF EXISTS ${dbName};`, function (err, result) {
        if (err) {
            console.log(err);
            return;
        };
    ...
    dbAssignment.query(createCommentsTable, function (err, result) {
        ...
        let commentsData = "INSERT INTO `comments` VALUES (1,'I agree',1,'2019-08-30 08:44:03'),(2,'Haha I gotta try that!',3,'2019-08-30 18:26:09');";
         dbAssignment.query(commentsData, function (err, result) {
            ...
            console.log("Data for comments created");
         });
      });
    };
    });  
...
var dropDatabase =  () => {
     dbAssignment.query(`DROP DATABASE ${dbName};`, function (err, result) {
            if (err) throw err;
            console.log(result);
